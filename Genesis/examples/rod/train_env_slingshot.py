...
class Train_Env_Slingshot(Train_Env):
    def __init__(self, task='wiring', log_dir="xxx/wiring", n_envs=5, GUI=False):
        # control vertices (indices on the rope)
        self.control_idx = [6]

        super().__init__(task, n_envs=n_envs, log_dir=log_dir, GUI=GUI)

        # RL/vectorized env configuration (mirror Train_Env_Wiring_ring)
        self._backend = 'numpy'

        # Observation / action specs
        self._obs_dim = self.rope.n_vertices * 3
    ...
    def construct_scene(self):
        ...
        self._rope_base_pos = np.array([0.0, 0.0, 0.21], dtype=np.float32)
    ...
    def step_all(self, env_mask, action):
        ...
        for j in range(self._steps_per_action):
            ...
            target_ctrl = current_ctrl + masked_delta * interp * 0.3
