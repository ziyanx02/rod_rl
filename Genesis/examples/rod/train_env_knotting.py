...
class Train_Env_Knotting(Train_Env):
    def __init__(self, task='wiring', log_dir="xxx/wiring", n_envs=5, GUI=False):
        ...
        # RL/vectorized env configuration (aligned with Train_Env_Wiring_ring)
        self._backend = 'numpy'
        ...
        # Optional target positions for shaping reward
        self.target_pos = None
        try:
            target_path = "target_pos/knotting_finalpos.npy"
            if os.path.exists(target_path):
                self.target_pos = np.load(target_path)
                print(f'Loaded target pos from "{target_path}", shape = {self.target_pos.shape}')
        except Exception as e:
            print(f"Warning: failed to load target positions for knotting: {e}")
    ...
    def construct_scene(self):
        ...
        self._rope_base_pos = np.array([-0.7, 0.15, 0.02], dtype=np.float32)
    ...
    def reward(self):
        # If a target configuration is provided, match it
        if self.target_pos is not None:
            verts_batch = self.rope.get_all_verts()  # [n_envs, n_verts, 3]
            if verts_batch.shape[1] == self.target_pos.shape[0]:
                rewards = []
                ...
                print(f"Warning: target_pos shape {self.target_pos.shape} mismatches verts {verts_batch.shape}")

        # Fallback: neutral reward to keep API working
        return [0.0 for _ in range(self.n_envs)]
